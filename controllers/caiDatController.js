...
exports.getCaiDat = async (req, res, next) => {
  try {
    ...
    if (!caiDat) {
      ...
      let isUpdated = false;
      if (caiDat.heroSub === undefined) { caiDat.heroSub = caiDat.tenDonVi || ''; isUpdated = true; }
      ...
      if (caiDat.soChiDoanTrucThuoc === undefined) { caiDat.soChiDoanTrucThuoc = 10; isUpdated = true; }
      if (isUpdated) {
        await caiDat.save();
      }
    }
